...
/**
 * 简单测试commons-lang的数据 ArrayUtils – 用于对数组的操作，如添加、查找、删除、子数组、倒序、元素类型转换等； BitField
 * – 用于操作位元，提供了一些方便而安全的方法； BooleanUtils – 用于操作和转换boolean或者Boolean及相应的数组；
 * CharEncoding – 包含了Java环境支持的字符编码，提供是否支持某种编码的判断； CharRange – 用于设定字符范围并做相应检查；
 * CharSet – 用于设定一组字符作为范围并做相应检查； CharSetUtils – 用于操作CharSet； CharUtils –
 * 用于操作char值和Character对象； ClassUtils – 用于对Java类的操作，不使用反射； ObjectUtils –
 * 用于操作Java对象，提供null安全的访问和其他一些功能； RandomStringUtils – 用于生成随机的字符串；
 * SerializationUtils – 用于处理对象序列化，提供比一般Java序列化更高级的处理能力； StringEscapeUtils –
 * 用于正确处理转义字符，产生正确的Java、JavaScript、HTML、XML和SQL代码； StringUtils –
 * 处理String的核心类，提供了相当多的功能； SystemUtils –
 * 在java.lang.System基础上提供更方便的访问，如用户路径、Java版本、时区、操作系统等判断； Validate –
 * 提供验证的操作，有点类似assert断言； WordUtils – 用于处理单词大小写、换行等。
 */
public class TestCommonsLang {
	 ...
	 * 
	 * public static boolean isAlpha(String str); 只由字母组成 public static boolean
	 * isAlphaSpace(String str); 只有字母和空格组成 public static boolean
	 * isAlphanumeric(String str);只由字母和数字组成 public static boolean
	 * isAlphanumericSpace(String str);只由字母数字和空格组成 public static boolean
	 * isNumeric(String str);只由数字组成 public static boolean isNumericSpace(String
	 * str);只由数字和空格组成
	 */
	@Test
	...
	/**
	 * 针对ObjectUtils本处放弃apache-commons-lang包 采用spring-util包里面的ObjectUtils
	 * org.springframework.util.ObjectUtils 常用方法:ObjectUtils.isEmpty(Object obj)
	 * ObjectUtils.isEmpty(Object[] array) ObjectUtils.isArray(Object obj)
	 * //Check whether the given array contains the given element.
	 * ObjectUtils.containsElement(Object[] array, Object element) //Check
	 * whether the given array of enum constants contains a constant with the
	 * given name, ignoring case when determining a match.
	 * ObjectUtils.containsConstant(Enum<?>[] enumValues, String constant)
	 * //Check whether the given array of enum constants contains a constant
	 * with the given name, ignoring case when determining a match.
	 * ObjectUtils.containsConstant(Enum<?>[] enumValues, String constant,
	 * boolean caseSensitive) //Return whether the given throwable is a checked
	...
	/*
	 * org.apache.commons.lang.math 在Jakarta
	 * Commons中，专门处理数学计算的类分别可以在两个地方找到：一是Commons
	 * Lang的org.apache.commons.lang.math包中，二是在Commons
	 * Math这个单独的子项目中。由于后者主要是处理复数、矩阵等，相对使用比较少，在我的笔记中就只简单讲讲Commons
	 * Lang中的math包。对后者感兴趣的可以看看 http://jakarta.apache.org/commons/math/
	 * 
	 * org.apache.commons.lang.math包中共有10个类，这些类可以归纳成四组： 1- 处理分数的Fraction类； 2-
	 * 处理数值的NumberUtils类； 3-
	 * 处理数值范围的Range、NumberRange、IntRange、LongRange、FloatRange、DoubleRange类； 4-
